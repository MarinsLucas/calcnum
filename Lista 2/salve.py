...
def gauss(A, B, n):
	...
	for i in range(n):
		...
		for j in range(i + 1, n):
			razao = np.float64(A[j][i] / A[i][i])

			for k in range(n + 1):
				A[j][k] = np.float64(A[j][k] - razao * A[i][k])

	# Substituição regressiva
	solucao[n - 1] = np.float64(A[n - 1][n] / A[n - 1][n - 1])

	for i in range(n - 2, -1, -1):
		solucao[i] = np.float64(A[i][n])

		for j in range(i + 1, n):
			solucao[i] = np.float64(solucao[i] - A[i][j] * solucao[j])

		solucao[i] = np.float64(solucao[i] / A[i][i])
	...
	for k in range(n):
		...
		for i in range(k + 1, n):
			...
			for j in range(k + 1, n + 1):
				A[i][j] = np.float64(A[i][j] - f * A[k][j])
